...
AS
      ...
      LOOP
         ...
         BEGIN
            SELECT NVL (
             ...
             WHERE     con.document_status NOT IN ('2010', '2050')
                   AND con.contract_id = y.contract_id;
         EXCEPTION
            WHEN OTHERS
            THEN
               n_cur_real := 0;
   ...
   FUNCTION po_submit_request (pf_batch_id    NUMBER,
                               v_user_id      NUMBER,
                               pf_org_id      NUMBER)
      RETURN NUMBER
   IS
   ...
   BEGIN
      ...
      THEN
         ...
         EXCEPTION
            ...
            THEN
               write_log (
                     'Error on PO Interface Concurrent Request at  Memo Header ID =  '
                  || pf_batch_id
                  || ' '
                  || SQLERRM,
                  'log');
      ...
      ELSE
         write_log (
               'Can not Find Buer ID on PO Interface Concurrent Request at  Memo Header ID =  '
            || pf_batch_id,
            'log');
         RETURN (0);
      END IF;
   ...
   IS
      ...
      IS
         SELECT /*+ CPU_COSTING  */
          ...
          WHERE     buyer.employee_id = usert.employee_id
   ...
   PROCEDURE get_location (v_inv_org   IN     NUMBER,
   ...
   AS
      ...
      IS
           SELECT /*+ CPU_COSTING  */
                  ...
                  (SELECT grade_control_flag
                    ...
                    WHERE     inventory_item_id = hd.item_id
                          AND organization_id = hd.ou_org_id)
                     grade_control
             /*,
                                                                                            dt.RECEIPT_ID*/
             -- By pcn @ 100630
            ...
            WHERE     hd.memo_header_id = dt.memo_header_id
                  ...
                  AND (   NVL (dt.po_status, 'N') <> 'Y'
                       ...
                       OR ( (SELECT DECODE (NVL (rec.consign_flag, 'N'),
                                            'Y', NVL (dt.consign_status, 'E'),
                                            'N')
                               FROM von_receipt rec
                              WHERE rec.receipt_id = dt.receipt_id)) NOT IN ('Y',
                                                                             'N') -- End Add for Support Consignment Return By pcn @ 100524
                                                                                 )
                  --
      ...
      IS
         SELECT /*+ CPU_COSTING  */
          ...
          WHERE     receipt.receipt_id = dt.receipt_id
                AND dt.memo_header_id = headerid;
   ...
   BEGIN
      ...
      BEGIN
         SELECT /*+ CPU_COSTING  */
           ...
           FROM hr_organization_units_v org,
                hr_organization_information_v inf
          WHERE     inf.organization_id = org.organization_id
                AND inf.org_information1_meaning = 'Operating Unit'
      ...
      THEN
         BEGIN
            SELECT /*+ CPU_COSTING  */
                  SUBSTR (batch_name, 1, 14),
                   TO_NUMBER (SUBSTR (batch_name, -3)) + 1
              INTO v_batch1, v_batch2
              FROM ap_batches_all
             WHERE     batch_date =
      ...
      BEGIN
         nrownum_display :=
            NVL (
               p_memo_row,
               count_total_memo (p_org_id     => p_org_id,
                                 p_memo_num   => NVL (p_memo_num, 11)));
      EXCEPTION
         WHEN OTHERS
         THEN
      ...
      FOR one IN cur_hd
      LOOP
         EXIT WHEN     NOT (nrownum <= NVL (p_memo_row, 10))
                   AND NOT (nrownum <= 10);

         /*2 End if*/

         write_log ('########## Begin Program  PO Interface- ' || CHR (13),
                    'log');
         ...
         -- *********** npp
         -- Checkg Memo Date
         IF    apps.inv_txn_manager_pub.get_open_period (one.inv_org_id,
                                                         one.memo_date,
                                                         1) = 0
            OR apps.inv_txn_manager_pub.get_open_period (one.inv_org_id,
                                                         one.memo_date,
                                                         1)
                  IS NULL
         THEN
            ...
            THEN
               ...
               apps.von_pormtopo.write_log (
                  'Receipt Date and Current Date not in Open Period.',
                  'log');
               GOTO end_one_loop;
            END IF;
         END IF;
         ...
         THEN
            write_log (
                  'PO_STATUS not equal ''Y'' and Memo Header ID = '
               || one.memo_header_id,
               'log');
            ...
            -- Get location
            get_location (v_inv_org   => one.inv_org_id,
                          loc_code    => v_location,
                          loc_id      => v_line_location_id);

            IF check_memoid_in_po_headers (v_memo_id => one.memo_header_id) =
                  'N'
            THEN
               /*1 Clear interface table*/
               clear_interface_table (p_id      => one.memo_header_id,
                                      vstatus   => v_clear_status);
            ...
            ELSE
               ...
               BEGIN
                  SELECT /*+ CPU_COSTING  */
                    ...
                    INTO v_po_header_id
                    FROM po_headers poh
                   WHERE     poh.attribute10 = v_batch_id
                         AND poh.attribute11 = 'PORM';
            ...
            LOOP
               ...
               -- *********** npp
               -- Checkg Receipt Date
               IF    apps.inv_txn_manager_pub.get_open_period (
                        two.inv_org_id,
                        two.receipt_date,
                        1) = 0
                  OR apps.inv_txn_manager_pub.get_open_period (
                        two.inv_org_id,
                        two.receipt_date,
                        1)
                        IS NULL
               ...
               UPDATE von_purchase_memo_dt dt
                  SET dt.po_interface_header_id = v_line_id,
                      dt.po_interface_line_id = v_header_id
           ...
           <<end_two_loop>>
            ...
            v_max_wait := 0;
            v_request_id :=
               po_submit_request (pf_batch_id   => v_batch_id,
                                  v_user_id     => userid,
                                  pf_org_id     => v_org_id);
            COMMIT;
                  ...
                  ELSE
                     ...
                     BEGIN
                        SELECT /*+ CPU_COSTING  */
                         ...
                         WHERE     hd_inf.process_code = 'ACCEPTED'
                               AND hd_inf.attribute10 = v_batch_id
                     ...
                     THEN
                        ...
                        EXCEPTION
                           ...
                           THEN
                              write_log (
                                    'Can not update table von_purchase_memo_hd'
                                 || CHR (13)
                                 || SQLERRM,
                                 'Log');
                              ROLLBACK;
                     ...
                     ELSE
                        BEGIN
                           SELECT    'PO Interface_Transaction_Id => '
                        ...
                        EXCEPTION
                           ...
                           THEN
                              ...
                              THEN
                                 write_log (
                                    'Error But Can not find error message in PO_INTERFACE_ERRORS',
                                    'log');
                              END IF;
                        END;
            ...
            LOOP
               ...
               THEN
                  BEGIN
                     UPDATE /*+ CPU_COSTING  */
                           von_purchase_memo_hd
                        SET po_number = v_po_num,
                            po_interface_header_id = v_header_id
                      WHERE memo_header_id = x.memo_header_id;
                  EXCEPTION
                     WHEN OTHERS
                     THEN
                        write_log (
                           'can not update po number to ' || one.memo_number,
                           'log');
                  END;
               END IF;
               ...
               LOOP
                  ...
                  THEN
                     ...
                     write_log ('PO Number รอบ 2 : ' || v_po_num,
                                'log');
                     ...
                     BEGIN
                        SELECT /*+ CPU_COSTING  */
                          ...
                          INTO v_line_num
                          FROM po_lines_all
                         WHERE     attribute10 = x.memo_line_id
                               AND po_header_id = y.po_header_id; -- by PCN 100630
                     EXCEPTION
                        WHEN OTHERS
                        THEN
                           v_line_num := NULL;
                     ...
                     THEN
                        ...
                        EXCEPTION
                           ...
                           THEN
                              write_log (
                                    'Can not update ''Y'' to memo line id '
                                 || x.memo_line_id
                                 || ' memo number '
                                 || one.memo_number,
                                 'log');
                  ...
                  THEN
                     ...
                     EXCEPTION
                        ...
                        THEN
                           write_log (
                                 'Can not update PO status memo number '
                              || one.memo_number,
                              'log');
                     END;
                  END IF;
            ...
            FOR rcv
               IN (SELECT /*+ CPU_COSTING  */
                    ...
                    WHERE     dt.memo_header_id = one.memo_header_id
                          ...
                          AND NVL (dt.rcv_status, 'N') <> 'Y')
            LOOP
               BEGIN
                  write_log (
                        'Org id : '
                     || one.org_id                             --rcv.ou_org_id
                     || ', User id : '
                     || userid
                     || ', PO number : '
                     || rcv.po_number,
                     'log');
                  apps.von_pormtopo.receive_po (
                     x_org_id             => one.org_id,
                     userid               => userid,
         ...
         FOR consign
            IN (SELECT /*+ CPU_COSTING  */
                 ...
                 WHERE     dt.memo_header_id = one.memo_header_id
                       ...
                       AND ( (SELECT DECODE (NVL (rec.consign_return, 'N'),
                                             'Y', 'Y',
                                             NVL (dt.consign_status, 'E'))
                                FROM von_receipt rec
                               WHERE rec.receipt_id = dt.receipt_id)) NOT IN ('Y',
                                                                              'N') -- fIX BUG @ 29 Jul 10
                       AND ( (SELECT DECODE (
                                        NVL (rec.consign_flag, 'N'),
                                        'Y', NVL (dt.consign_status, 'E'),
                                        'N')
                                FROM von_receipt rec
                               WHERE rec.receipt_id = dt.receipt_id)) NOT IN ('Y',
         ...
         LOOP
            BEGIN
               write_log (
                  ...
                  || consign.receipt_id,
                  'log');
               apps.von_pormtopo.consign_return_memo (
                  x_org_id             => one.org_id,
                  x_userid             => userid,
               ...
               von_pormtopo.write_log (
                  ' End Consign Return : result=' || v_cns_return_status,
                  'log');
            END;
         END LOOP consign;
         ...
         IF     NVL (one.ap_inv_status, 'N') <> 'Y'
            AND get_po_status (one.memo_header_id) = 'Y'
            AND get_rcv_status (one.memo_header_id) = 'Y'
         THEN
            apps.von_pormtopo.von_invoices (
               errbuf          => errbuf,
               retcode         => retcode,
               userid          => userid,
      ...
      apps.von_pormtopo.write_log (
            '--------------------------------------------------------------------------------------------- Total '
         || TO_CHAR (ngrand_total_inv_amt, '999,999,999.99'),
         'line');
      ...
      IS
           SELECT /*+ CPU_COSTING  */
                 a.po_number,
                  ...
                  (SELECT segment1
                     FROM mtl_system_items
                    WHERE     inventory_item_id = a.item_id
                          AND organization_id = a.to_organization_id)
                     item_number,
                  ...
                  (  pov.quantity
                   - (pov.quantity_received - pov.quantity_billed)
                   - pov.quantity_cancelled
                   - pov.quantity_billed)
                     remainding_qty
             FROM rcv_enter_receipts_supplier_v a, po_line_locations pov
            WHERE     receipt_source_code IN ('INVENTORY',
                                              'INTERNAL ORDER',
                                              'VENDOR')
                  AND ( (   a.source_type_code = 'VENDOR'
                         OR a.source_type_code = 'ASN'
                         OR a.source_type_code = 'INTERNAL'))
                  AND ( (NVL (a.closed_code, 'OPEN') NOT IN ('CLOSED',
      ...
      IS
         SELECT /*+ CPU_COSTING  */
           ...
           FROM von_purchase_memo_dt dt,
                von_purchase_memo_hd hd,
                von_receipt rc
          WHERE     dt.memo_line_id = line_memo_id
                AND dt.memo_header_id = hd.memo_header_id
                AND dt.receipt_id = rc.receipt_id;
   ...
   BEGIN
      ...
      LOOP
         ...
         apps.von_pormtopo.write_log ('Entering Receiving Insert Procedure',
                                      'log');
         ...
         BEGIN
            ...
            apps.von_pormtopo.write_log (
               ...
               || one.po_line_id,
               'log');
            apps.von_pormtopo.write_log ('Line memo id = ' || v_line_memo_id,
                                         'log');
         EXCEPTION
            WHEN NO_DATA_FOUND
            THEN
            ...
            THEN
               apps.von_pormtopo.write_log (
                     'Check po_lines_all where po_header_id='
                  || one.po_header_id
                  || ' and PO_LINE_ID='
                  || one.po_line_id
                  || '.',
         ...
         LOOP
            ...
            THEN
               apps.von_pormtopo.write_log (
                     'Delete rcv_headers_interface with receipt_num : '
                  || next_rec_num,
                  'log');
            ...
            THEN
               apps.von_pormtopo.write_log (
                     'Delete  rcv_transactions_interface with interface_transaction_id : '
                  || memo.rcv_interface_transaction_id,
                  'log');
               ...
               apps.von_pormtopo.write_log ('Delete Complete!', 'log');
               apps.von_pormtopo.write_log (
                     'Delete  mtl_transaction_lots_interface with transaction_interface_id : '
                  || memo.inv_transaction_interface_id,
                  'log');
            ...
            -- Peroid Receipt Date is Close
            IF    apps.inv_txn_manager_pub.get_open_period (memo.inv_org_id,
                                                            memo.memo_date,
                                                            1) = 0
               OR apps.inv_txn_manager_pub.get_open_period (memo.inv_org_id,
                                                            memo.memo_date,
                                                            1)
                     IS NULL
            ...
            SELECT /*+ CPU_COSTING  */
              ...
              FROM von_purchase_memo_hd
             WHERE     (   rcv_interface_groups_id IS NOT NULL
                        OR rcv_headers_interface_id IS NOT NULL)
                   AND memo_header_id = v_memo_header_id;

            apps.von_pormtopo.write_log (
                  'Count Interface group id : '
               || v_chk_interface_transaction_id,
               'log');
            ...
            THEN
               ...
               BEGIN
                  SELECT *
                   ...
                   WHERE     receipt_num = next_rec_num
                         AND ship_to_organization_id = one.to_organization_id;
               ...
               apps.von_pormtopo.write_log (
                  '<B># RECEIVE NUMBER : </B> : ' || next_rec_num,
                  'log');
               ...
               apps.von_pormtopo.write_log ('Group id : ' || v_group_id,
                                            'log');
               ...
               apps.von_pormtopo.write_log (
                  'Header interface id : ' || v_header_interface_id,
                  'log');
               apps.von_pormtopo.write_log (
                  '########## Insert RCV_HEADERS_INTERFACE ##########',
                  'log');
               ...
               COMMIT;
               apps.von_pormtopo.write_log (
                  '########## Insert RCV_HEADERS_INTERFACE Complete! ##########',
                  'log');

               BEGIN
                  UPDATE /*+ CPU_COSTING  */
                        von_purchase_memo_hd
                     SET rcv_interface_groups_id = v_group_id,
                         rcv_number = next_rec_num,
                         rcv_headers_interface_id = v_header_interface_id
                   WHERE memo_header_id = v_memo_header_id;

                  v_rcv_number := next_rec_num;
                  apps.von_pormtopo.write_log (
                     'update memo header id : ' || v_memo_header_id,
                     'log');
               EXCEPTION
                  WHEN OTHERS
                  THEN
            ...
            apps.von_pormtopo.write_log (
               '########## Insert RCV_TRANSACTIONS_INTERFACE ##########',
               'log');
            ...
            apps.von_pormtopo.write_log ('item number= ' || one.item_number,
                                         'log');
            ...
            THEN
               ...
               apps.von_pormtopo.write_log (
                  '########## Insert MTL_TRANSACTION_LOTS_INTERFACE ##########',
                  'log');
               ...
               apps.von_pormtopo.write_log (
                  '##### Update VON_PURCHASE_MEMO_DT #####',
                  'log');

               UPDATE /*+ CPU_COSTING  */
                     von_purchase_memo_dt
                  SET inv_transaction_interface_id =
                         mtl_material_transactions_s.CURRVAL
                WHERE memo_line_id = memo.memo_line_id;
            ELSE
               apps.von_pormtopo.write_log ('#####No lot control.#####',
                                            'log');
            END IF;
      ...
      THEN
         IF fnd_concurrent.wait_for_request (v_req_id,
                                             ...
                                             v_message)
         THEN
            von_pormtopo.write_log (v_dev_status || ' : ' || v_message,
                                    'log waiting 1');
            ...
            ELSE
               ...
               THEN
                  von_pormtopo.write_log (
                        'Please check query in rcv_headers_interface with condition ''where header_interface_id= '
                     || v_header_interface_id
                     || '''',
                     'log');
                  GOTO end_now;
      ...
      FOR two
         IN (SELECT /*+ CPU_COSTING  */
                   dt.rcv_interface_transaction_id,
                    dt.memo_line_id,
                    hd.rcv_headers_interface_id
               FROM von_purchase_memo_dt dt, von_purchase_memo_hd hd
              WHERE     dt.memo_header_id = v_memo_header_id
                    AND hd.memo_header_id = dt.memo_header_id)
      LOOP
         apps.von_pormtopo.write_log (
            '########## Check for update Memo Receive status ##########',
            'log');
         apps.von_pormtopo.write_log (
               'interface transaction id : '
            || two.rcv_interface_transaction_id
            || ' Memo line id : '
            || two.memo_line_id,
            'log');
         ...
         apps.von_pormtopo.write_log (
            'Transactions_interface found ' || v_chk_interface,
            'log');
         ...
         apps.von_pormtopo.write_log (
            'Receive Transactions found ' || v_chk_transaction,
            'log');
         ...
         ELSE
            ...
            EXCEPTION
               WHEN OTHERS
               THEN
                  apps.von_pormtopo.write_log ('Can not find error message',
                                               'line');
            END;
      ...
      IF     v_final_chk_all <> 0
         AND v_final_chk_y <> 0
         AND v_final_chk_y = v_final_chk_all
      THEN
         ...
         SELECT /*+ CPU_COSTING  */
               ou_org_id,
                ...
                (SELECT loc.inventory_location_id
                  ...
                  WHERE     loc.organization_id = rc.inv_org_id
                        AND loc.segment1 = rc.locator)
                   locator_id,
                rc.vendor_id,
                (SELECT DISTINCT hd.memo_date
                  ...
                  WHERE hd.memo_header_id = cons.memo_header_id)
                   memo_date,
                cons.ROWID r_id
           FROM von_purchase_memo_dt cons,
                von_receipt rc,
                mtl_system_items mi
          WHERE     cons.memo_line_id = x_memo_line_id
                AND rc.receipt_id = cons.receipt_id
                AND rc.item_id = mi.inventory_item_id
                AND rc.inv_org_id = mi.organization_id;
      ...
      LOOP
         ...
         EXCEPTION
            ...
            THEN
               rcv_error := rcv_error + 1;
               x_return_status :=
                  'To Many Transaction Types Setup for Consignment';
               GOTO end_now;
         END;
         ...
         --  Checkg Receipt Date
         -- Peroid Receipt Date is Close
         IF    apps.inv_txn_manager_pub.get_open_period (x.inv_org_id,
                                                         x.memo_date,
                                                         1) = 0
            OR apps.inv_txn_manager_pub.get_open_period (x.inv_org_id,
                                                         x.memo_date,
                                                         1)
                  IS NULL
        ...
        */
         ...
         BEGIN
            ...
            SELECT /*+ CPU_COSTING  */
             ...
             WHERE     whse_code = x.whse_code
                   AND acc_type = 1040
                   AND item_id = x.item_id
                   AND ou_org_id = x.ou_org_id;
         EXCEPTION
            WHEN NO_DATA_FOUND
            THEN
               vsegment1 := NULL;
         ...
         THEN
            ...
            THEN
               BEGIN
                  SELECT /*+ CPU_COSTING  */
                   ...
                   WHERE     organization_id = x.ou_org_id
                         AND inventory_item_id = x.item_id
                         AND lot_number =
                                   TO_CHAR (x.receipt_date, 'YYMMDD')
                                || '/'
                                || x.receipt_number;
               ...
               EXCEPTION
                  ...
                  THEN
                     ...
                     apps.von_opm.create_lot (
                        p_orgn_id         => x.inv_org_id,
                        p_item_id         => x.item_id,
                        p_lot_no          =>    TO_CHAR (x.receipt_date, 'YYMMDD')
                                             || '/'
                                             || x.receipt_number,
                        p_grade           => x.grade,
               ...
               EXCEPTION
                  ...
                  THEN
                     ...
                     ELSE
                        ...
                        g_cns_status_result :=
                           'not Lot Control : Error !!' || p_msg_data;
                        --                        DBMS_OUTPUT.put_line (p_msg_data);
                        rcv_error := rcv_error + 1;
                        x_return_status := p_msg_data;
                        GOTO end_now;
            ...
            apps.von_pormtopo.write_log (
                  'Chk Point 2 consign_return_memo  !! : '
               || g_cns_status_result,
               'LOG');                                               --- today
         ...
         THEN
            g_cns_status_result := 'not Lot Control : Success !!';
            apps.von_pormtopo.write_log (
               'Chk Point 3 In consign_return_memo not Lot Control : Success !! : ',
               'LOG');                                               --- today
            COMMIT;
         END IF;
         ...
         SELECT /*+ CPU_COSTING  */
               rcv_hd.shipment_header_id,
                ...
                rcv_trans.currency_code,
                  NVL (rcv_trans.quantity, 0)
                * NVL (rcv_trans.po_unit_price, 0)
                   amount,
                rcv_hd.receipt_num,
                ...
                rcv_hd.attribute10
           FROM rcv_shipment_headers rcv_hd,
                rcv_shipment_lines rcv_ln,
                rcv_transactions rcv_trans
   ...
   BEGIN
      apps.von_pormtopo.write_log ('##########Start AP Interface########',
                                   'log');
      ...
      FOR one IN memo_hd
      LOOP
         apps.von_pormtopo.write_log ('receive number ' || one.rcv_number,
                                      'log');
         ...
         BEGIN
            SELECT /*+ CPU_COSTING  */
             ...
             WHERE     inf.organization_id = org.organization_id
                   AND inf.org_information1_meaning = 'Operating Unit'
                   AND org.organization_id = one.ou_org_id;
         ...
         IF     one.total_deduct_amount IS NOT NULL
            AND one.total_deduct_amount > 0
         THEN
            v_addition_line := v_addition_line + 1;
            apps.von_pormtopo.write_log (
               '#########Index : ' || v_addition_line || '##########',
               'log');
            ...
            EXCEPTION
               ...
               THEN
                  apps.von_pormtopo.write_log (
                     'Error not found code combination id',
                     'log');
            END;
         ...
         IF     one.total_retention_amount IS NOT NULL
            AND one.total_retention_amount > 0
         THEN
            v_addition_line := v_addition_line + 1;
            apps.von_pormtopo.write_log (
               '#########Index : ' || v_addition_line || '##########',
            ...
            apps.von_pormtopo.write_log (
               ...
               'log');
            v_amount (v_line_type (v_addition_line)) :=
               one.total_retention_amount;
            v_addition_amount :=
               v_addition_amount + one.total_retention_amount;
            apps.von_pormtopo.write_log (
                  'Index : '
               || v_addition_line
               || ' Line Type : '
         ...
         BEGIN
            ...
            BEGIN
               SELECT vendor_site_code
                ...
                WHERE     vendor_site_id = one.vendor_site_id
                      AND vendor_id = one.vendor_id
               ...
               THEN
                  BEGIN
                     SELECT vendor_site_id
                       ...
                       FROM po_vendor_sites_all
                      WHERE     vendor_site_code = one.vendor_site_code
                            AND vendor_id = one.vendor_id
                            AND inactive_date IS NULL
                            AND vendor_site_id = one.vendor_site_id;
            ...
            */
            BEGIN
               DELETE FROM ap_invoices_interface
                     WHERE     attribute10 = one.memo_header_id
                           AND attribute11 = 'PORM';
            ...
            ninvoice_amount := one.total_memo_amount - v_addition_amount;
            ntotal_invoice_amount :=
               NVL (ntotal_invoice_amount, 0) + ninvoice_amount;
               ...
               EXCEPTION
                  ...
                  THEN
                     ...
                     EXCEPTION
                        ...
                        THEN
                           apps.von_pormtopo.write_log (
                                 'No Header_id found on receipt number : '
                              || one.rcv_number
                              || ' Please check in rcv_shipment_headers.',
                              'log');
                           GOTO end_novalidate;
                     END;
                  WHEN NO_DATA_FOUND
         ...
         apps.von_pormtopo.write_log (
            'RCV Shipment Header id : ' || v_shipment_header_id,
            'log');
         ...
         LOOP
            BEGIN
               SELECT /*+ CPU_COSTING  */
                ...
                WHERE     po_distribution_id = two.po_distribution_id
                      AND po_header_id = two.po_header_id
                      AND po_line_id = two.po_line_id;
            EXCEPTION
               WHEN OTHERS
               THEN
                  v_dist_code_combination_id := NULL;
                  apps.von_pormtopo.write_log (
                     'Error PO distribution code combination id .',
                     'log');
                  GOTO endlooptwo;
            END;
            ...
            apps.von_pormtopo.write_log ('Memo line id: ' || two.attribute10,
                                         'log');
            ...
            BEGIN
               SELECT /*+ CPU_COSTING  */
                ...
                WHERE     hd.memo_header_id = dt.memo_header_id
                      AND dt.memo_line_id = two.attribute10;
            EXCEPTION
               WHEN OTHERS
               THEN
                  v_line_desc := 'error';
                  ...
                  THEN
                     BEGIN
                        ...
                        EXCEPTION
                           ...
                           THEN
                              apps.von_pormtopo.write_log (
                                 'CAN NOT UPDATE AP_INVOICE_LINES_ALL',
                                 'LOG');
                        END;
                        ...
                        EXCEPTION
                           ...
                           THEN
                              apps.von_pormtopo.write_log (
                                 'CAN NOT UPDATE AP_INVOICES_ALL',
                                 'LOG');
                        END;
                        ...
                        apps.von_pormtopo.write_log (
                           ...
                           || one.memo_number
                           || ' ----------------------- ยอด '
                           || TO_CHAR (ninvoice_amount, '999,999,999.99'),
                           'line');
                  ...
                  ELSE
                     v_sts :=
                        fnd_concurrent.set_completion_status (
                           ...
                           'Check output file for warnings');
                     apps.von_pormtopo.write_log (
                           'Error: Invoice Interface on Memo: '
                        || one.memo_number
                        || ') ',
                        'LOG');
                     ...
                     UPDATE /*+ CPU_COSTING  */
                           von_purchase_memo_dt
                        SET ap_inv_status = v_program_status,
                            ap_inv_number = one.memo_number
                      WHERE memo_header_id = one.memo_header_id;
                  ...
                  THEN
                     ...
                     EXCEPTION
                        ...
                        THEN
                           v_invoice_id := -1;
                           apps.von_pormtopo.write_log (
                                 'CAN NOT FIND INVOICE ID (INVOICE NUMBER '
                              || one.memo_number
                              || ') ',
                              'LOG');
                     END;
   ...
   AS
      ...
      IS
         SELECT /*+ CPU_COSTING  */
          ...
          WHERE     document_status = 1020
                AND NVL (consign_flag, 'E') <> 'Y'
                AND receipt_id = c_line_id
                AND rc.item_id = mi.inventory_item_id
                AND rc.inv_org_id = mi.organization_id;
      ...
      LOOP
         ...
         THEN
            DELETE /*+ CPU_COSTING  */
                  mtl_transaction_lots_interface
             WHERE transaction_interface_id =
                      rec.con_transaction_interface_id;
         ...
         BEGIN
            ...
            SELECT /*+ CPU_COSTING  */
              ...
              INTO vsegment1,
                   ...
                   ndistribution_account_id
              FROM von_acc_mapping
             WHERE     whse_code = rec.whse_code
                   AND acc_type = 1040
                   AND item_id = rec.item_id
                   AND ou_org_id = rec.ou_org_id;
         EXCEPTION
            WHEN NO_DATA_FOUND
            THEN
               vsegment1 := NULL;
         ...
         -- Checkg Receipt Date
         IF    apps.inv_txn_manager_pub.get_open_period (rec.inv_org_id,
                                                         rec.receipt_date,
                                                         1) = 0
            OR apps.inv_txn_manager_pub.get_open_period (rec.inv_org_id,
                                                         rec.receipt_date,
                                                         1)
                  IS NULL
         THEN
         ...
         IF get_mtl_duplicate_rows (rec.receipt_number,
                                    rec.receipt_id,
                                    rec.receipt_id,
                                    nreceipt_date,
                                    rec.sub_inventory) = 0
         ...
         /*+ CPU_COSTING  */
         INSERT
           INTO mtl_transactions_iface_logs (transaction_interface_id,
                                             source_code,
                                             source_line_id,
                                             source_header_id,
                                             process_flag,
            ...
            THEN
               BEGIN
                  SELECT /*+ CPU_COSTING  */
                    ...
                    INTO l_lot_rec
                    FROM mtl_lot_numbers
                   WHERE     organization_id = rec.ou_org_id
                         AND inventory_item_id = rec.item_id
                         AND lot_number =
                                   TO_CHAR (rec.receipt_date, 'YYMMDD')
                                || '/'
                                || rec.receipt_number;
               ...
               EXCEPTION
                  ...
                  THEN
                     ...
                     apps.von_opm.create_lot (
                        p_orgn_id         => rec.inv_org_id,
                        p_item_id         => rec.item_id,
                        p_lot_no          =>    TO_CHAR (rec.receipt_date, 'YYMMDD')
                                             || '/'
                                             || rec.receipt_number,
                        p_grade           => rec.grade,
                        p_date            => rec.receipt_date,
                        p_conv            => TO_CHAR (
                                               rec.net_weight / rec.gross_weight),
         ...
         THEN
            ...
            THEN
               ...
               THEN
                  von_pormtopo.write_log (
                     'Return ->  Please Check on Transaction Interface output file for warnings',
                     'log');
                  v_sts :=
                     fnd_concurrent.set_completion_status (
                        'ERROR',
                        ' Please Check on Transaction Interface output file for warnings');
                  rcv_error := NVL (rcv_error, 0) + 1;
                  rcv_complete := 0;
                  GOTO end_now;
               ELSE
                  ...
                  IF    v_process_status IS NOT NULL
                     OR x_return_status IS NOT NULL
                  THEN
                     von_pormtopo.write_log (
                        'Return -> Record Error:' || ' ' || x_return_status,
                        'log');
                     rcv_error := NVL (rcv_error, 0) + 1;
                     rcv_complete := 0;
                     GOTO end_now;
                  ELSE
                     ...
                     THEN
                        ...
                        ELSE
                           ...
                           ELSE
                              von_pormtopo.write_log (
                                 'Return -> Record does not process  ',
                                 'log');
                              x_return_status :=
                                    'Record does not process '
                                 || v_process_status
                                 || ' , '
                                 || x_return_status;
                              rcv_error := NVL (rcv_error, 0) + 1;
               ...
               END IF;
            ELSE
               von_pormtopo.write_log ('Return -> Request ID = 0 or Null',
                                       'log');
        ...
        <<end_now>>
         von_pormtopo.write_log (
            'Check 2 # rcv_complete_waiting => ' || rcv_complete,
            'log');
